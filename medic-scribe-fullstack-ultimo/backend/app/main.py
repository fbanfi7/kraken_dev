...
from fastapi import FastAPI, UploadFile, File, HTTPException, status, Form, WebSocket, WebSocketDisconnect, BackgroundTasks
...
######## Diccionario para almacenar las conexiones activas por usuario
...
async def websocket_endpoint(websocket: WebSocket, user_id: str):
    ...
    try:
        ...
        # Eliminar conexión al desconectarse
        active_connections[user_id].remove(websocket)



######### Activar el websocket para notificar al usuario cuando la nota SOAP esté lista
async def notify_user_summary_ready(user_id: str, soap_note: str):
    connections = active_connections.get(user_id, [])
    print(f"[WebSocket] Notificando a {len(connections)} conexiones para user_id={user_id}")
...
async def transcribe_and_summarize_audio(audio_file: UploadFile = File(...), prompt_instruction: str = Form(None), user_id: str = Form(...)):
    """
    ...
    try:
        ...
        await notify_user_summary_ready(user_id= user_id, soap_note=soap_note)
