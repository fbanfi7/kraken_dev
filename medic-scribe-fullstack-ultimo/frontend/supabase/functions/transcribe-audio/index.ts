  ...
  try {
    ...
    if (!result.soap_note) {
      throw new Error('No SOAP note received from API');
        ...
        // Send WebSocket message with the generated note content
    try {
      const wsResponse = await fetch(`http://localhost:8000/ws/${user.id}`, {
        method: 'POST',
        headers: {
      ...
      if (!wsResponse.ok) {
        console.error('Failed to send WebSocket message:', await wsResponse.text());
      } else {
